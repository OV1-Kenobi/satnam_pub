...
const json = (statusCode: number, body: ApiResponse, headers?: Record<string,string>) => ({
  statusCode,
  headers: {
    ...buildCorsHeaders(),
    ...(headers || {})
...
function buildCorsHeaders(): Record<string,string> {
  ...
  const allowedOrigin = isProd ? (process.env.FRONTEND_URL || 'https://www.satnam.pub') : '*';
...
async function setRlsContext(supabase: any, ownerHash: string): Promise<void> {
  try {
    ...
    try {
      await supabase.rpc('set_app_config', { setting_name: 'app.current_user_hash', setting_value: ownerHash, is_local: true });
    } catch {
      try {
        await supabase.rpc('app_set_config', { setting_name: 'app.current_user_hash', setting_value: ownerHash, is_local: true });
      } catch {}
    }
  }
}
...
async function requireSession(event: any) {
  ...
  const session = await SecureSessionManager.validateSessionFromHeader(typeof authHeader === 'string' ? authHeader : undefined);
...
async function handleVerify(event: any) {
  ...
  const body = parseJSON<{ tagUID?: string; encryptedResponse?: string; challengeData?: string }>(event.body);
  if (!body || !body.tagUID) {
    return json(400, { success: false, error: 'Missing tagUID' });
  }

  const supabase = getRequestClient((event.headers?.authorization || event.headers?.Authorization || '').replace(/^Bearer\s+/i, ''));
  ...
  try {
    ...
    if (!duidSecret) return json(500, { success: false, error: 'Server configuration error' });
    hashedTag = createHmac('sha256', duidSecret).update(`${session.hashedId}:${body.tagUID}`).digest('hex');
  } catch {
  ...
  const { data: reg, error } = await supabase
    ...
    .select('id, owner_hash, hashed_tag_uid, uid, family_role, encrypted_config')
  ...
  return json(200, { success: true, data: { sessionMetadata: { role: reg.family_role || 'private' } } });
}
...
async function handleRegister(event: any) {
  ...
  const body = parseJSON<{ tagUID?: string; encryptedMetadata?: string; familyRole?: string }>(event.body);
  ...
  return json(200, { success: true, data: { tagId: data.id, hashedTag: data.hashed_tag_uid } });
}
...
async function handlePreferences(event: any) {
  ...
  if (method === 'GET') {
    ...
    if (error) return json(200, { success: true, data: { preferences: null, registeredTags: tags || [] } });
    return json(200, { success: true, data: { preferences: prefs || null, registeredTags: tags || [] } });
  }

  if (method === 'POST' || method === 'PUT') {
    const body = parseJSON<{ preferences?: { nfc_pin_timeout_seconds?: number; nfc_require_confirmation?: boolean; preferred_method?: string; fallback_method?: string } }>(event.body);
    ...
    if (typeof body.preferences.nfc_pin_timeout_seconds === 'number') {
      updates.nfc_pin_timeout_seconds = body.preferences.nfc_pin_timeout_seconds;
    }
    if (typeof body.preferences.nfc_require_confirmation === 'boolean') {
      updates.nfc_require_confirmation = body.preferences.nfc_require_confirmation;
    }
    if (typeof body.preferences.preferred_method === 'string') updates.preferred_method = body.preferences.preferred_method;
    if (typeof body.preferences.fallback_method === 'string') updates.fallback_method = body.preferences.fallback_method;
    ...
    if (error) return json(500, { success: false, error: 'Failed to save preferences' });
    return json(200, { success: true, data: { preferences: data } });
  }

  if (method === 'DELETE') {
    const body = parseJSON<{ tagId?: string }>(event.body);
    if (!body || !body.tagId) return json(400, { success: false, error: 'Missing tagId' });
    ...
    if (error) return json(500, { success: false, error: 'Failed to delete tag' });
