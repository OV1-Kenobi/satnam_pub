// Netlify (active) Function: /api/individual/wallet
// Delegates to API implementation at api/individual/wallet.js

export const handler = async (event, context) => {
  try {
    const mod = await import('../../api/individual/wallet.js');
    const fn = (mod && (mod.default || mod.handler));
    if (typeof fn !== 'function') {
      return { statusCode: 500, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ success: false, error: 'Individual wallet handler not available' }) };
    }
