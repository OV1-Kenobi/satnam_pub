// Netlify (active) Function: /api/auth/nip07-challenge
// Self-contained implementation to avoid cross-directory imports and bundling issues

export const handler = async (event) => {
  // CORS setup
  const getAllowedOrigin = (origin) => {
    const isProd = process.env.NODE_ENV === 'production';
    if (isProd) return 'https://www.satnam.pub';
    if (!origin) return '*';
    try {
  ...
  try {
    ...
    const domain = process.env.VITE_APP_DOMAIN || event.headers?.host || 'localhost:3000';
