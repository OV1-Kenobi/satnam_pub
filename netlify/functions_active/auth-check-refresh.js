// Netlify (active) Function: /api/auth/check-refresh -> /.netlify/functions/auth-check-refresh
// Wraps the API route implementation (Express-style) with a Netlify handler

export const handler = async (event, context) => {
  try {
    const mod = await import('../../api/auth/check-refresh.js');
    const fn = (mod && (mod.default || mod.handler));
    if (typeof fn !== 'function') {
      return { statusCode: 500, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ success: false, error: 'Check-refresh handler not available' }) };
    }
