...
import { generateBrowserFingerprint } from '../crypto/privacy-manager';
...
const badgeAwardSchema = {
  ...
  recipientPubkey: { type: 'string', required: true, minLength: 64, maxLength: 64 },
  evidence: {
    type: 'object',
    required: true,
    properties: {
  ...
  privacyLevel: { type: 'string', enum: ['public', 'family', 'private'], required: true },
  mentorVerification: {
    type: 'object',
    optional: true,
    properties: {
...
const wotNotarizationSchema = {
  ...
  studentPubkey: { type: 'string', required: true, minLength: 64, maxLength: 64 },
  mentorPubkey: { type: 'string', required: true, minLength: 64, maxLength: 64 },
...
const mentorRegistrationSchema = {
  ...
  verificationLevel: { type: 'string', enum: ['basic', 'intermediate', 'advanced'], required: true },
...
const performAntiGamingChecks = async (
  ...
  try {
    ...
    if (recentActions && recentActions.length > 0) {
      ...
      const timeSinceLastAction = now - new Date(lastAction.created_at).getTime();
      if (timeSinceLastAction < 30 * 1000) { // 30 seconds minimum
    ...
    if (userProfile) {
      const daysSinceRegistration = (now - new Date(userProfile.created_at).getTime()) / (1000 * 60 * 60 * 24);
      if (daysSinceRegistration < 7) { // New user within 7 days
...
export const handler: Handler = async (event) => {
  try {
    ...
    const rateLimitResult = await rateLimiter.checkLimit(rateLimitKey, BADGE_RATE_LIMITS.award);
...
const handleAwardBadge = async (data: any, headers: any) => {
  ...
  const { badgeId, recipientPubkey, evidence, privacyLevel, mentorVerification } = data;
  ...
  try {
    ...
    const availableBadges = badges?.filter(badge => {
      ...
      if (badge.prerequisites?.length > 0 && progress) {
        const earnedBadges = progress.badges_earned?.map((b: any) => b.badge_id) || [];
      ...
      if (badge.criteria?.time_requirements?.minimum_study_hours && progress) {
        if (progress.total_study_hours < badge.criteria.time_requirements.minimum_study_hours) {
          return false;
        }
      }

      // Check verification requirements
      if (badge.criteria?.verification_requirements?.guardian_approval_required && !familyId) {
        return false;
      }
...
const handleGetStudentDashboard = async (data: any, headers: any) => {
  ...
  try {
    ...
    const [progress, availableBadges, achievementsSummary, mentorInteractions, credentializations] = await Promise.all([
      handleGetStudentProgress({ studentPubkey }, headers).then(res => JSON.parse(res.body).data),
      handleGetAvailableBadges({ studentPubkey, familyId }, headers).then(res => JSON.parse(res.body).data),
...
const handleWoTNotarization = async (data: any, headers: any) => {
  ...
  const { badgeId, studentPubkey, mentorPubkey, verificationNotes, privacyLevel, guardianApproval } = data;
  ...
  try {
    ...
    const notarization = {
      ...
      verification_hash: await hashString(`${badgeId}${studentPubkey}${mentorPubkey}`),
...
const handleMentorRegistration = async (data: any, headers: any) => {
  ...
  const { mentorPubkey, nip05, competencyAreas, verificationLevel, bio, institutionAffiliation, yearsExperience } = data;
...
const getAchievementsSummary = async (studentPubkey: string) => {
  try {
    ...
    const wotVerifiedBadges = progress.badges_earned.filter((b: any) => b.wot_verified).length;
    const institutionalCosignedBadges = progress.badges_earned.filter((b: any) => b.institutional_cosigned).length;
