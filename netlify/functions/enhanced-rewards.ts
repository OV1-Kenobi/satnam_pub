...
import { generateBrowserFingerprint } from '../../src/lib/privacy/browser-fingerprint';
 ...
 */
async function checkRapidSubmissions(studentPubkey: string): Promise<{ allowed: boolean; reason?: string }> {
  const now = Date.now();
 ...
 */
async function checkTimeBetweenActions(studentPubkey: string): Promise<{ allowed: boolean; reason?: string }> {
  ...
  if (timeSinceLastAction < MIN_TIME_BETWEEN_ACTIONS) {
    return { 
      allowed: false, 
      reason: `${Math.round((MIN_TIME_BETWEEN_ACTIONS - timeSinceLastAction) / 1000)}s too soon` 
    };
  }
 ...
 */
async function getAvailableRewards(studentPubkey: string, familyId?: string): Promise<RewardResponse> {
  try {
    ...
    for (const config of configs) {
      const validation = await validateRewardEligibility(config, progress, studentPubkey, familyId);
...
async function validateRewardEligibility(
  ...
  // Check study time
  if (config.minimum_study_time && progress.total_study_time < config.minimum_study_time) {
    violations.push(`Insufficient study time: ${progress.total_study_time}/${config.minimum_study_time} minutes`);
    riskScore += 15;
  }
  ...
  if (config.max_invitations_per_day) {
    const todayInvitations = getTodayInvitations(progress.invitation_history || []);
 ...
 */
async function getStudentRedemptions(studentPubkey: string): Promise<RewardResponse> {
  try {
...
async function redeemReward(
  ...
  try {
    ...
    const eligibility = await validateRewardEligibility(config, progress, studentPubkey);
    ...
    const redemption = {
      ...
      status: config.family_approval_required && !guardianApproval ? 'pending' : 'approved',
      created_at: Date.now(),
      privacy_encrypted: config.privacy_level === 'encrypted',
      browser_fingerprint: browserFingerprint,
      course_id: config.currency === 'course_credits' ? config.course_id : undefined,
    ...
    if (browserFingerprint && progress.browser_fingerprints) {
      const updatedFps = [...(progress.browser_fingerprints || []), browserFingerprint];
      await supabase
        .from('student_progress')
...
export const handler: Handler = async (event, context) => {
  ...
  try {
    ...
    // Parse request
    const request: RewardRequest = event.httpMethod === 'GET' 
      ? { 
    ...
    switch (request.action) {
      case 'available':
        response = await getAvailableRewards(request.studentPubkey, request.familyId);
        break;
