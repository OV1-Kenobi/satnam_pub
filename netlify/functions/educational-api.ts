...
import { rateLimit } from '../security/rate-limiter';
...
const courseRegistrationSchema = {
  ...
  enrollmentType: { type: 'string', required: true, enum: ['immediate', 'approval-required', 'external'] },
  provider: { type: 'string', required: true, enum: ['satnam', 'citadel-academy'] },
...
async function handleCourseRegistration(data: any, res: any) {
  try {
    ...
    const { courseId, userPubkey, familyId, enrollmentType, provider, cost, metadata } = data;
    ...
    if (enrollmentType === 'approval-required' && course.max_students) {
      ...
      if (currentEnrollment && currentEnrollment >= course.max_students) {
        return res.status(409).json({ error: 'Course enrollment limit reached' });
      }
    }
    ...
    return res.status(200).json({
      success: true,
      enrollment,
      message: enrollmentType === 'immediate' ? 'Successfully enrolled' : 'Enrollment request submitted'
    });
...
async function handleProgressUpdate(data: any, res: any) {
  try {
    ...
    const { courseId, userPubkey, moduleId, progress, timeSpent, quizScore, completedAt } = data;
    ...
    const moduleProgressData = {
      ...
      completed_at: completedAt || (progress === 100 ? Math.floor(Date.now() / 1000) : null),
      updated_at: Math.floor(Date.now() / 1000)
    };

    const { error: moduleError } = await supabase
      .from('module_progress')
      .upsert([moduleProgressData], { onConflict: 'course_id,user_pubkey,module_id' });
...
async function updateOverallProgress(courseId: string, userPubkey: string) {
  try {
    ...
    const quizScores = moduleProgress.filter(m => m.quiz_score !== null).map(m => m.quiz_score);
...
async function updateCognitiveCapital(userPubkey: string, courseId: string) {
  try {
    ...
    const baseScore = course.difficulty === 'beginner' ? 100 : 
                     course.difficulty === 'intermediate' ? 200 : 300;
    ...
    const newMetrics: CognitiveCapitalMetrics = {
      ...
      total_time_spent: (currentMetrics?.total_time_spent || 0) + (course.duration * 60),
      average_quiz_score: currentMetrics?.average_quiz_score || 0, // Will be recalculated
      badges_earned: (currentMetrics?.badges_earned || 0) + course.badges.length,
      certificates_earned: (currentMetrics?.certificates_earned || 0) + 1,
      cognitive_capital_score: (currentMetrics?.cognitive_capital_score || 0) + baseScore,
...
async function handleGetCognitiveCapital(data: any, res: any) {
  try {
    ...
    if (error && error.code !== 'PGRST116') { // PGRST116 = no rows returned
      ...
      return res.status(500).json({ error: 'Failed to fetch cognitive capital metrics' });
    }
    ...
    if (!metrics) {
      ...
      if (createError) {
        ...
        return res.status(500).json({ error: 'Failed to create cognitive capital metrics' });
      }
...
async function handleCompleteCourse(data: any, res: any) {
  try {
    ...
    if (!courseId || !userPubkey) {
      return res.status(400).json({ error: 'Course ID and user pubkey are required' });
    }
    ...
    if (enrollmentError) {
      ...
      return res.status(500).json({ error: 'Failed to complete course enrollment' });
    }
    ...
    if (progressError) {
      ...
      return res.status(500).json({ error: 'Failed to complete course progress' });
    }
...
async function handleAwardBadge(data: any, res: any) {
  try {
    ...
    if (!userPubkey || !badgeId) {
      return res.status(400).json({ error: 'User pubkey and badge ID are required' });
    }
  ...
  try {
    ...
    if (userPubkey && pathways) {
      for (const pathway of pathways) {
        ...
        const completedCourses = pathwayCourses?.filter(c => c.status === 'completed').length || 0;
        pathway.progress = Math.round((completedCourses / pathway.courses.length) * 100);
        pathway.status = pathway.progress === 100 ? 'completed' : 
                        pathway.progress > 0 ? 'in-progress' : 'not-started';
      }
    }
