...
export class NTAG424ProductionManager {
  ...
  constructor(supabaseClient?: any, lightningClient?: LightningClient, phoenixdClient?: PhoenixdClient) {
  ...
  private getMasterKey(): string {
    ...
    const devKey = import.meta.env.VITE_NTAG424_MASTER_KEY || 'dev-ntag424-master-key-32-chars';
    console.warn('⚠️ Using development NTAG424 master key. Use Supabase Vault in production.');
    return devKey;
  }
  ...
  private encryptConfig(config: NTAG424ProductionConfig): string {
    try {
      ...
      const encrypted = CryptoJS.AES.encrypt(configString, this.masterKey).toString();
      return encrypted;
    } catch (error) {
  ...
  async authenticateProductionTag(
    ...
    try {
      ...
      const sunValid = await this.verifySUNMessage(sunMessage, config.aesKeys.sun);
      if (!sunValid) {
        return { success: false, error: "Invalid SUN message - potential replay attack" };
      }

      // Generate session token
      const sessionToken = this.generateProductionSessionToken(uid, config.userNpub);
   ...
   */
  async executeTapToSign(operation: NTAG424SignOperation): Promise<string | null> {
    try {
   ...
   */
  private async verifySUNMessage(sunMessage: string, sunKey: string): Promise<boolean> {
    try {
   ...
   */
  private generateProductionSessionToken(uid: string, userNpub: string): string {
   ...
   */
  private async getWalletAccess(uid: string, config: NTAG424ProductionConfig): Promise<any> {
   ...
   */
  private async setupLightningInfrastructure(uid: string, role: string, limits?: any): Promise<void> {
    try {
   ...
   * Setup Voltage Lightning node
   */
  private async setupVoltageNode(uid: string, role: string): Promise<{ success: boolean; nodeId?: string }> {
    try {
   ...
   * Setup PhoenixD wallet
   */
  private async setupPhoenixdWallet(uid: string, role: string): Promise<{ success: boolean; walletId?: string }> {
    try {
   ...
   * Verify operation signature
   */
  private async verifyOperationSignature(operation: NTAG424SpendOperation | NTAG424SignOperation): Promise<boolean> {
    try {
   ...
   * Check spending limits
   */
  private async checkSpendingLimits(operation: NTAG424SpendOperation): Promise<boolean> {
    try {
      ...
      const weeklySpent = await this.getWeeklySpending(operation.uid, weekStart);
   ...
   */
  private async executeLightningPayment(operation: NTAG424SpendOperation): Promise<boolean> {
    try {
   ...
   */
  private async executeECashPayment(operation: NTAG424SpendOperation): Promise<boolean> {
    try {
   ...
   */
  private async executeFedimintPayment(operation: NTAG424SpendOperation): Promise<boolean> {
    try {
   ...
   */
  private async generateNostrSignature(operation: NTAG424SignOperation): Promise<string> {
    try {
      ...
      const event = {
        ...
        tags: [['p', operation.uid], ['t', 'ntag424-sign']]
      };
   ...
   */
  private async generateTransactionSignature(operation: NTAG424SignOperation): Promise<string> {
    try {
   ...
   */
  private async generateCommunicationSignature(operation: NTAG424SignOperation): Promise<string> {
    try {
   ...
   */
  private async generateRecoverySignature(operation: NTAG424SignOperation): Promise<string> {
    try {
   ...
   */
  private async generateIdentitySignature(operation: NTAG424SignOperation): Promise<string> {
    try {
  ...
  private async getDailySpending(uid: string, date: string): Promise<number> {
    try {
      const { data } = await this.supabase
        ...
        .gte("timestamp", date)
        .lt("timestamp", new Date(new Date(date).getTime() + 24 * 60 * 60 * 1000).toISOString());

      return data?.reduce((total: number, op: any) => {
   ...
   */
  private async getWeeklySpending(uid: string, weekStart: string): Promise<number> {
    try {
      const { data } = await this.supabase
        .from("ntag424_operations_log")
        .select("metadata")
        ...
        .lt("timestamp", new Date(new Date(weekStart).getTime() + 7 * 24 * 60 * 60 * 1000).toISOString());
...
export async function testNTAG424ProductionModule(): Promise<{
  ...
  try {
    ...
    try {
      ...
      results.tests.keyGeneration = testKey.length === 64 && /^[0-9a-f]{64}$/.test(testKey);
      console.log('✅ Key generation test:', results.tests.keyGeneration ? 'PASSED' : 'FAILED');
    } catch (error) {
    ...
    try {
      ...
      results.tests.encryption = JSON.stringify(testConfig) === JSON.stringify(decrypted);
      console.log('✅ Encryption test:', results.tests.encryption ? 'PASSED' : 'FAILED');
    } catch (error) {
    ...
    try {
      ...
      console.log('✅ PIN hashing test:', results.tests.pinHashing ? 'PASSED' : 'FAILED');
    } catch (error) {
    ...
    results.success = Object.values(results.tests).every(test => test === true);
