...
export function useClientDecryption(): UseClientDecryptionReturn {
   ...
   *
   * No longer requires knownValues - all profile data is encrypted and reversible
   */
  const decryptProfile = useCallback(
    async (encryptedData: EncryptedUserData): Promise<DecryptedUserProfile> => {
      setIsDecrypting(true);
      setDecryptionError(null);

      try {
        console.log("🔓 Starting client-side profile decryption...");
        const decryptedProfile = await decryptUserProfile(encryptedData);
   ...
   */
  const decryptProfiles = useCallback(
    async (
      encryptedData: EncryptedUserData[]
    ): Promise<DecryptedUserProfile[]> => {
      setIsDecrypting(true);
      ...
      try {
        console.log(
          `🔓 Starting batch decryption of ${encryptedData.length} profiles...`
        );
        ...
        const errorMessage =
          error instanceof Error
            ? error.message
            : "Unknown batch decryption error";
...
export function useDecryptedCurrentUser() {
  const [decryptedProfile, setDecryptedProfile] =
    useState<DecryptedUserProfile | null>(null);
  ...
  const fetchAndDecryptProfile = useCallback(async () => {
    ...
    try {
      ...
      const decrypted = await decryptProfile(
        encryptedProfile as EncryptedUserData
      );
      ...
      console.error(
        "❌ Failed to fetch and decrypt current user profile:",
        errorMessage
      );
...
export function useDecryptedUserProfile(userId: string | null) {
   ...
   */
  const fetchAndDecryptProfile = useCallback(
    async (id: string) => {
      setIsLoading(true);
      setError(null);
      ...
      try {
        ...
        console.error(
          "❌ Failed to fetch and decrypt user profile:",
          errorMessage
        );
        setError(errorMessage);
        setDecryptedProfile(null);
