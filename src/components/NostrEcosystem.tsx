...
const NostrEcosystem: React.FC<NostrEcosystemProps> = ({ onBack, userIdentity = "yourname@satnam.pub" }) => {
  ...
  const categories = [
    { id: 'mobile', name: 'Mobile Apps', description: 'Native mobile experiences' },
    { id: 'web', name: 'Web Clients', description: 'Browser-based interfaces' },
    { id: 'social', name: 'Social Networks', description: 'Connect and communicate' },
    { id: 'media', name: 'Media & Streaming', description: 'Share and discover content' },
    { id: 'publishing', name: 'Publishing', description: 'Write and share articles' },
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-purple-900 rounded-2xl p-6 mb-8 border border-yellow-400/20">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-3xl font-bold text-white">Next Steps: Use Your New Decentralized Identity</h1>
                <p className="text-purple-200">Explore the Nostr ecosystem with your sovereign identity</p>
              </div>
            </div>
        ...
        <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-8 mb-8 border border-white/20">
          <div className="text-center mb-8">
            <div className="w-20 h-20 bg-gradient-to-br from-yellow-400 to-orange-500 rounded-full flex items-center justify-center mx-auto mb-6">
              <img src="/SatNam.Pub logo.png" alt="SatNam.Pub" className="h-12 w-12 rounded-full" />
            </div>
            <h2 className="text-2xl font-bold text-white mb-4">Welcome to the Nostr-verse!</h2>
            <p className="text-purple-100 text-lg max-w-4xl mx-auto leading-relaxed">
              Now that you've forged your sovereign Nostr identity, put it to use! Your 'True Name' will be carried with you wherever you go in the Nostr-verse. 
              Here are the best tools and apps to connect, create, and explore the Nostr ecosystem. Then return to the Citadel Academy once you have the lay of the land, to learn more and cultivate your family's cognitive capital.
            </p>
          </div>

          {/* Identity Display */}
          <div className="bg-white/10 rounded-2xl p-6 mb-8">
            <h3 className="text-white font-bold text-lg mb-4 text-center">Your Sovereign Identity</h3>
            <div className="flex items-center justify-center space-x-4">
              <div className="bg-white/10 rounded-lg p-4 flex-1 max-w-md">
                <p className="text-purple-200 text-sm mb-2">Your NIP-05 Identity:</p>
                <p className="text-yellow-400 font-mono text-lg">{userIdentity}</p>
              </div>
              <button
              ...
              >
                {copiedIdentity ? <CheckCircle className="h-5 w-5" /> : <Copy className="h-5 w-5" />}
        ...
        <div className="bg-gradient-to-r from-yellow-400/20 to-orange-500/20 rounded-2xl p-8 mb-8 border border-yellow-400/50">
          <div className="flex items-start space-x-6">
            ...
            <div className="flex-1">
              <h3 className="text-yellow-400 font-bold text-xl mb-3">Essential: Browser Key Management</h3>
              <h4 className="text-white font-bold text-lg mb-2">nos2x (Browser Extension)</h4>
              <p className="text-yellow-100 mb-4">
                Securely manage your Nostr private key (nsec) in your browser. Lets you sign events and log in to Nostr web apps without exposing your private key, supporting NIP-07 and recommended by the Nostr creator.
              </p>
              <a
        ...
        <div className="space-y-8">
          {categories.map((category) => {
            const categoryApps = nostrApps.filter(app => app.category === category.id);
            if (categoryApps.length === 0) return null;

            return (
              <div key={category.id} className="bg-white/10 backdrop-blur-sm rounded-2xl p-8 border border-white/20">
                <div className="flex items-center space-x-3 mb-6">
                  <div className={`w-12 h-12 bg-gradient-to-br ${categoryColors[category.id as keyof typeof categoryColors]} rounded-full flex items-center justify-center text-white`}>
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {categoryApps.map((app) => (
                    <div key={app.id} className="bg-white/10 rounded-xl p-6 hover:bg-white/15 transition-all duration-300 border border-white/20">
                      <div className="flex items-start space-x-4 mb-4">
                        <div className={`w-12 h-12 bg-gradient-to-br ${categoryColors[app.category]} rounded-full flex items-center justify-center text-white flex-shrink-0`}>
                          {app.icon}
                        </div>
                        <div className="flex-1">
        ...
        <div className="bg-gradient-to-r from-purple-900/80 to-blue-900/80 backdrop-blur-sm rounded-2xl p-8 mt-8 border border-yellow-400/30">
          <div className="text-center">
            ...
            <p className="text-purple-100 text-lg mb-6 max-w-3xl mx-auto">
              Already have a favorite Nostr client? Just add your new <span className="font-mono text-yellow-400">{userIdentity}</span> reusable human-readable ID to your profile (into the NIP-05 settings for your profile). 
              Set yourself up to start receiving 'Zapped\' LN bitcoin payments into your new wallet and begin engaging financially and socially with your peers!
            </p>
        ...
        <div className="text-center mt-12">
          <div className="flex flex-col sm:flex-row items-center justify-center space-y-2 sm:space-y-0 sm:space-x-8 text-purple-200">
            <span className="flex items-center space-x-2">
              <img src="/SatNam.Pub logo.png" alt="SatNam.Pub" className="h-4 w-4 rounded-full" />
