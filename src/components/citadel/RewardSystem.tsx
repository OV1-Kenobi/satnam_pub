...
import {
  ...
  RewardType,
  rewardSystem
} from '../../lib/citadel/reward-system.js';
...
type ViewType = 'available' | 'history' | 'pending' | 'anti-gaming';

// Helper functions for reward icons and colors
const getRewardIcon = (type: RewardType) => {
  const icons: Record<RewardType, React.ComponentType<any>> = {
    'lightning-sats': Zap,
...
const getRewardColor = (type: RewardType): string => {
...
const RewardSystem: React.FC<RewardSystemProps> = ({
  ...
  const loadRewardsData = async () => {
    try {
      ...
      // Try enhanced reward system first, fallback to basic API
      try {
        ...
        // Fallback to basic API calls for backward compatibility
        console.warn('Enhanced reward system unavailable, falling back to basic API:', enhancedError);

        // Load available rewards via API
        ...
        // Load redemption history via API
        ...
        // Clear anti-gaming validations for basic mode
        setAntiGamingValidations(new Map());
      }
    } catch (error) {
  ...
  const handleRedeemReward = async (rewardType: RewardType) => {
    try {
      ...
      // Check anti-gaming validation if available
      ...
      try {
        // Try enhanced reward system first
        redemption = await rewardSystem.redeemReward(studentPubkey, rewardType);
      } catch (enhancedError) {
        // Fallback to basic API for backward compatibility
        console.warn('Enhanced redemption unavailable, falling back to basic API:', enhancedError);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-400 via-purple-500 to-purple-600 p-4">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-purple-900 rounded-2xl p-6 mb-8 border border-yellow-400/20">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-purple-200">Earn Bitcoin rewards with advanced security protection</p>
            </div>
            <div className="flex items-center space-x-4">
        ...
        <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-6 mb-8 border border-white/20">
          <div className="flex space-x-1">
            {[
              ...
              ...(antiGamingValidations.size > 0 ? [{ key: 'anti-gaming', label: 'Security', icon: Shield, count: 0 }] : [])
        ...
        {currentView === 'anti-gaming' && antiGamingValidations.size > 0 && (
          <AntiGamingSection
...
const RewardCard: React.FC<RewardCardProps> = ({
  ...
  return (
    <div className={`bg-white/10 rounded-xl p-6 border transition-all duration-300 ${isBlocked
      ...
      <div className="text-center">
        ...
        {isAvailable && !isBlocked && (
          <button
            ...
            className={`w-full py-3 px-4 rounded-lg font-bold transition-all duration-300 ${isRedeeming
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-purple-900 rounded-2xl p-6 max-w-2xl w-full max-h-[90vh] overflow-y-auto border border-yellow-400/20">
        ...
        {redemption.reward_type === 'lightning-sats' && (redemption.status === 'processed' || redemption.status === 'approved') && (
