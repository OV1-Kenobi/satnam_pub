-- =====================================================
-- COMPLETE IDENTITY SYSTEM DATABASE MIGRATION - FIXED VERSION
...
-- =====================================================
-- SECTION 2: CREATE ALL TABLES FIRST (WITHOUT RLS)
-- Create tables without RLS to avoid policy conflicts during creation
-- =====================================================
...
-- Add relationships after all tables are created
-- =====================================================
...
BEGIN
    ...
    ELSE
        RAISE NOTICE 'Foreign key constraint profiles_family_id_fkey already exists';
    END IF;
END $$;

-- =====================================================
-- SECTION 5: UTILITY FUNCTIONS
-- Create helper functions before enabling RLS
-- =====================================================
...
-- =====================================================
-- SECTION 6: INSERT INITIAL DATA (BEFORE RLS)
-- Insert data while RLS is not yet enabled to avoid policy conflicts
-- =====================================================

-- Insert default families first (referenced by other tables)
...
DO $$
BEGIN
    -- Check if table exists and has the domain column
    IF EXISTS (
        SELECT 1 FROM information_schema.columns
        ...
        RAISE NOTICE 'Inserted default NIP-05 records successfully';
    ELSE
        RAISE WARNING 'nip05_records table or domain column not found, skipping data insertion';
    END IF;
END $$;

-- =====================================================
-- SECTION 7: ENABLE RLS AND CREATE POLICIES
-- Enable Row Level Security and create policies after data insertion
-- =====================================================
...
DROP POLICY IF EXISTS "nip05_records_insert_for_registration" ON nip05_records;
CREATE POLICY "nip05_records_insert_for_registration" ON nip05_records
    FOR INSERT
...
-- Allow inserts for registration
DROP POLICY IF EXISTS "profiles_insert_for_registration" ON profiles;
CREATE POLICY "profiles_insert_for_registration" ON profiles
    FOR INSERT
    WITH CHECK (true); -- Allow all inserts for now
...
-- Allow inserts for registration
DROP POLICY IF EXISTS "user_identities_insert_for_registration" ON user_identities;
CREATE POLICY "user_identities_insert_for_registration" ON user_identities
    FOR INSERT
    WITH CHECK (true); -- Allow all inserts for now
...
-- Grant permissions for anonymous users (limited - for registration)
...
GRANT INSERT ON nip05_records TO anon; -- For registration
...
-- =====================================================
-- SECTION 10: VERIFICATION AND COMPLETION
-- Verify the migration was successful and display results
-- =====================================================
...
-- Verify nip05_records table has domain column and data
DO $$
DECLARE
...
BEGIN
    ...
    IF domain_column_exists THEN
        SELECT COUNT(*) INTO record_count FROM nip05_records;
        RAISE NOTICE 'SUCCESS: nip05_records table has domain column with % records', record_count;
    ELSE
        RAISE WARNING 'WARNING: nip05_records table missing domain column';
    END IF;
END $$;
...
BEGIN
    RAISE NOTICE '';
    RAISE NOTICE '🎉 COMPLETE IDENTITY SYSTEM DATABASE MIGRATION COMPLETED SUCCESSFULLY! (FIXED VERSION)';
    RAISE NOTICE '';
    RAISE NOTICE '📋 SUMMARY OF CHANGES:';
    RAISE NOTICE '  • Updated privacy_users table with privacy_level and zero_knowledge_enabled columns';
    RAISE NOTICE '  • Created nip05_records table for NIP-05 verification (with domain column)';
    ...
    RAISE NOTICE '  • Registration-friendly insert policies';
    ...
    RAISE NOTICE '';
    RAISE NOTICE '⚠️  IMPORTANT: The "domain" column error has been fixed by:';
    RAISE NOTICE '  • Creating tables before inserting data';
    RAISE NOTICE '  • Adding proper existence checks before data insertion';
    RAISE NOTICE '  • Enabling RLS after data insertion to avoid policy conflicts';
    RAISE NOTICE '';
