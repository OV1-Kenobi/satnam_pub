// scripts/generate-family-config.js
...
class FamilyFederationConfigGenerator {
  /**
   * Generate a secure family federation ID
   */
  generateFamilyFederationId() {
  ...
  generateInviteCode() {
    ...
    return `invite_${timestamp}_${randomSuffix}`;
  }

  /**
   * Create the complete family federation configuration
   */
  createFamilyFederation() {
   ...
   */
  generateEnvConfig() {
    const config = this.createFamilyFederation();
...
FEDIMINT_FAMILY_INVITE_CODE=${config.inviteCode}
    `.trim();
  }
}
