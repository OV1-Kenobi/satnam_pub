...
const TYPE_SUGGESTIONS: TypeSuggestion[] = [
  {
...
const CONTEXT_TYPES: { [key: string]: string } = {
  config: "Config",
  request: "Request",
  response: "Response",
  error: "Error",
...
class TypeScriptAnyFixer {
   ...
   */
  private analyzeFile(filePath: string): AnyTypeIssue[] {
   ...
   */
  private generateSuggestedFix(context: string, fullLine: string): string {
   ...
   */
  private determineSeverity(context: string): "error" | "warning" {
   ...
   */
  private applyFixes(filePath: string, issues: AnyTypeIssue[]): boolean {
    try {
   ...
   */
  async run(projectRoot: string, autoFix: boolean = false): Promise<void> {
