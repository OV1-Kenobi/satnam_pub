...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const userInvitations =
      await db.models.educationalInvitations.getUserInvitations(hashedUserId);

    // Get referral statistics
    const referralStats = await db.models.courseCredits.getUserReferralStats(
      hashedUserId
    );

    // Get credit history
    const creditHistory = await db.models.courseCredits.getUserCreditHistory(
      hashedUserId
    );
    ...
    // Get base URL from Supabase Vault (Master Context compliant)
    const baseUrl = await getAppBaseUrl();

    // Format invitations data
    const recentInvitations = invitationsArray.map(
      (invitation) => {
        const now = new Date();
        ...
        return {
          ...
          personalMessage:
            invitation.invitation_data?.personalMessage ||
    ...
    const formattedCreditHistory = creditsArray.map(
      (credit) => ({
        id: credit.id || "",
