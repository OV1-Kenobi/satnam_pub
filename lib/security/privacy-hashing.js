...
export async function generateUserSalt() {
  try {
    ...
    console.error('Failed to generate user salt:', error);
...
export async function createHashedUserData(userData, userSalt = null) {
  ...
  console.log('🔍 Creating hashed user data:', {
  ...
  const hashedData = {
    ...
    hashed_username: userData.username ? (console.log('Hashing username:', userData.username, 'with salt:', !!salt), await hashUserData(userData.username, salt)) : null,
    hashed_bio: userData.bio !== undefined ? (console.log('Hashing bio:', userData.bio, 'with salt:', !!salt), await hashUserData(userData.bio || '', salt)) : null,
    hashed_display_name: userData.displayName ? (console.log('Hashing displayName:', userData.displayName, 'with salt:', !!salt), await hashUserData(userData.displayName, salt)) : null,
    hashed_picture: userData.picture !== undefined ? (console.log('Hashing picture:', userData.picture, 'with salt:', !!salt), await hashUserData(userData.picture || '', salt)) : null,
