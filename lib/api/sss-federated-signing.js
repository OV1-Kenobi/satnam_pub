...
export class SSSFederatedSigningAPI {
    ...
    try {
      ...
      for (const sig of signatures) {
        ...
        if (share && share.encryptedShare) {
          // Decrypt the share (in production, this would use guardian's private key)
          // For now, we'll use the encrypted share data directly
          guardianShares.push({
      ...
      const privateKeyHex = reconstructedNsec.startsWith('nsec')
        ? (await import('nostr-tools/nip19')).decode(reconstructedNsec).data
        : reconstructedNsec;
  ...
  async notifyGuardians(requestId, guardians, requestData) {
    try {
      console.log(`[SSS] Notifying ${guardians.length} guardians about signing request ${requestId}`);
      ...
      const results = await Promise.all(
        guardians.map(async (guardianPubkey) => {
          try {
            ...
            if (result.success) {
              console.log(`[SSS] Guardian ${guardianPubkey} notified successfully`);
            } else {
              console.error(`[SSS] Failed to notify guardian ${guardianPubkey}:`, result.error);
            }

            return result;
          } catch (error) {
            console.error(`[SSS] Error notifying guardian ${guardianPubkey}:`, error);
      ...
      console.log(`[SSS] Notified ${successCount}/${guardians.length} guardians successfully`);

    } catch (error) {
      console.error('[SSS] Error notifying guardians:', error);
    }
  }
