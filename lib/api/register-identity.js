...
async function generateNostrKeyPair() {
  try {
    console.log('🔑 Starting backend key generation...');
    ...
    console.log('✅ Private key hex conversion complete, length:', privateKeyHex.length);

    // Fix: Use correct API for noble-curves migration
    const publicKeyBytes = getPublicKey(privateKey, true); // Force compressed format
    console.log('✅ Public key generated, length:', publicKeyBytes.length);

    const publicKeyHex = Array.from(publicKeyBytes).map(b => b.toString(16).padStart(2, '0')).join('');
    console.log('✅ Public key hex conversion complete, length:', publicKeyHex.length);

    // Use correct npub encoding: remove compression prefix for 64-char format
    const publicKeyWithoutPrefix = publicKeyHex.slice(2);
    console.log('✅ Public key without prefix, length:', publicKeyWithoutPrefix.length);
